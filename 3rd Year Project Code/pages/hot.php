...
<h1 class="homepages">Hot</h1>
<!-- This page is used to show the hottest scoring items in the database (hottness is based on the score divided by time its been up), getHotFiles retrieves the items 
(sorting them by hottest scoring items) from the database, it loops through the retrieved items and displays them, the review function is used to add to the score if the love button is 
pressed (the love button only shows if a user is logged in), the button function matches the item_id and user_id so a user can only add one score per item, to display images 
correctly I check whether they are portrait or landscape judging by the image dimensions and display them accordingly, only items marked public are displayed on the frontpages, 
details about the item are also displayed next to the item such as name of the item and creator(the creator name also serves as a link to their personal page, the show button at the bottom of the 
page removes the love button if a user has already clicked it (on any frontpage) -->
<?php

	require_once('classes/users.classes.php');
    $userObj = new users($DBH); 
	
	$hotFiles1 = $userObj->getHotFiles();
	
	foreach ($hotFiles1 as $key => $value) {
	...
	$hotFiles = $userObj->getHotFiles();
	
	foreach ($hotFiles as $key => $value) {
